...
Create `packages/cloudcompare-mcp/server.py`. It must load `cloudcompare_mcp.server` from mandatory `CLOUDCOMPARE_MCP_SOURCE`, replace `handle_get_cloudcompare_info`, replace the broken `handle_compute_normals` and its exact `TOOLS` entry, and call upstream `main()`. The info replacement must use upstream `find_cloudcompare`, `_ok`, and `_err`; success contains `binary`, `platform`, `version`, and `ready: true`, while missing binary or version returns only a structured error. The normals replacement uses `-OCTREE_NORMALS {auto|radius} -MODEL {LS|QUADRIC|TRI}`, rejects legacy `knn`, and exposes a closed schema. It accepts only absolute, distinct paths and explicit cloud-output suffixes, securely opens an existing output parent component-by-component, writes into private random staging, rejects empty/multiple/unreadable outputs, and atomically publishes with a no-clobber dirfd hardlink after identity checks. It must preserve the source, roll back publication if the parent identity changes, clean staging on every pre-commit path, and report post-commit cleanup failures as warnings. Radius is positive and expressed in source coordinate units; prefer omitted radius and CloudCompare `auto` when the correct scale is unknown. Patching must fail before startup unless exactly one upstream `compute_normals` Tool exists.
