...
The local module imports the fetched upstream server without vendoring it and replaces only `handle_get_cloudcompare_info`. Upstream attempts the unsupported `CloudCompare --version` command and can report plugin output as a version while claiming readiness. The replacement uses upstream binary discovery and response helpers, reports the Nix package version, and returns a structured error unless both the binary and package version are available. Upstream constructs its call dispatch at request time, so replacing the module-level handler before `main()` starts changes only that tool.
