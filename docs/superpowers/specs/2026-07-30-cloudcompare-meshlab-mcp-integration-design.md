...
The local module imports the fetched upstream server without vendoring it and replaces `handle_get_cloudcompare_info` plus the broken `compute_normals` handler and tool definition. Upstream attempts the unsupported `CloudCompare --version` command and can report plugin output as a version while claiming readiness. The info replacement uses upstream binary discovery and response helpers, reports the Nix package version, and returns a structured error unless both the binary and package version are available. The normals replacement uses CloudCompare 2.13's `OCTREE_NORMALS` and `MODEL` commands, maps `TRIANGULATION` to `TRI`, disables auto-save, and writes into a random private staging directory bound through a securely opened destination dirfd. It requires one nonempty regular output with no sidecars, reopens the result for validation, verifies source and directory identities, and atomically hardlinks the staged file into place without clobbering an existing path. Staging is removed on both success and failure; post-commit cleanup failures are returned as warnings. Same-UID staging manipulation is explicitly outside the threat model. Its closed public schema does not expose unsupported KNN selection; radius is an optional positive value in the source cloud's coordinate units, and callers should prefer CloudCompare's automatic radius when the appropriate scale is unknown. The wrapper requires exactly one upstream `compute_normals` tool definition and fails before server startup otherwise. Upstream constructs its call dispatch at request time, so replacing both module-level handlers before `main()` starts activates the corrected implementations.
...
- CloudCompare normals outputs require an existing non-symlink parent path, use private staging and atomic no-clobber publication, and never write directly to the requested destination.
...
- Exercise the corrected normals tool with LS, QUADRIC, and TRIANGULATION models, including automatic and explicit source-coordinate-unit radii, then verify each output contains normals, the source hash is unchanged, no staging remains, and a repeated destination is not clobbered.
